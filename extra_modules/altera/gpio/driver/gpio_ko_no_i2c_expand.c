...
#define GPIO_HPS_NUM (29 + 29 + 27)
#define GPIO_EXPAND_NUM 32
...
static struct gpio_desc *hps_gpios[GPIO_HPS_NUM];
static struct gpio_desc *expand_gpios[GPIO_EXPAND_NUM];
									...
									size_t count, loff_t *offset)
{
	...
	for (i = 0, pgpio = pgpios; i < gpio_num; i++, pgpio++) {
		...
		if (gpio_d == GPIO_DIR_IN)
			...
			gpiod_set_value(gpio_desc, gpio_v);
	}
									...
									size_t count, loff_t *offset)
{
	...
	for (i = 0, pgpio = pgpios; i < gpio_num; i++, pgpio++) {
		gpio_i = pgpios[i].ind;
		gpio_desc = hps_gpios[gpio_i];
		gpio_v = gpiod_get_value(gpio_desc);
		pgpio->val = gpio_v;
	}
...
static int gpio_platform_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(pdev->dev.of_node, "gpio-num", &gpio_num);
	if (ret) {
		pr_err("get gpio num failed!\n");
		up(&g_dev_probe_sem);
		return -ERESTARTSYS;
	}
	...
	g_gpio_priv_data.gpio_num = gpio_num;
	sema_init(&g_gpio_priv_data.sem, 1);
	g_gpio_priv_data.count = 0;

	
	for (i = 0; i < gpio_num; i++) 
		hps_gpios[i] = devm_gpiod_get_index(&pdev->dev, "hps", i, GPIOD_ASIS);
...
static int gpio_platform_remove(struct platform_device *pdev)
{
	...
	for (i = 0; i < g_gpio_priv_data.gpio_num; i++) {
		devm_gpiod_put(&pdev->dev, hps_gpios[i]);
		hps_gpios[i] = NULL;
	}	
...
static struct of_device_id gpio_driver_dt_ids[] = {
	{ .compatible = "gpio_dev,driver-1.0"},
	{ /* end of table */ }
};
...
MODULE_DESCRIPTION("Hanglory 3.0 gpio operation driver-1.0");
device_unregister
