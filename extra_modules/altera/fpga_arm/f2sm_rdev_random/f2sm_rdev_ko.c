...
//offset is 0, not used
// count is N * 12KB, 
// random test : offset is count/3, align to register cmd, count + count/3 <= mem[buf_index].size;
static ssize_t
f2smr_ram_dev_write(struct file *fp,
		   ...
		   loff_t *offset)
{
	...
	//pr_info("f2smr_ram_dev_write enter\n");
	//pr_info("         fp = 0x%08X\n", (uint32_t)fp);
	//pr_info("user_buffer = 0x%08X\n", (uint32_t)user_buffer);
	//pr_info("      count = 0x%08X\n", (uint32_t)count);
	//pr_info("    *offset = 0x%08X\n", (uint32_t)*offset);
	//pr_info("    iobuf = 0x%08X\n", (uint32_t)dev->cur_io_buf);
	//pr_info("    max_offset = 0x%08X\n", (uint32_t)dev->p_uio_info->mem[0].size);

	//io_buf =  dev->cur_io_buf; //dev->p_uio_info->mem[0].internal_addr;
	...
	writepos = temp_count/3;
	if((temp_count + writepos) > max_offset)
	{
		pr_info("f2smr_ram_dev_write count too big error exit\n");
		return -ERESTARTSYS;
	}
	...
	//pr_info("f2smr_ram_dev_write 0x%08x, len %d\n",io_buf,temp_count);
	dev->write_byte_count += temp_count;
	//*offset += temp_count;

	up(&dev->sem);
	//pr_info("f2smr_ram_dev_write exit\n");
	return count;
}
...
static ssize_t
f2smr_ram_dev_read(struct file *fp, char __user *user_buffer,
		  size_t count, loff_t *offset)
{
	...
	//pr_info("f2smr_map_dev_read enter\n");
	//pr_info("         fp = 0x%08X\n", (uint32_t)fp);
	//pr_info("user_buffer = 0x%08X\n", (uint32_t)user_buffer);
	//pr_info("      count = 0x%08X\n", (uint32_t)count);
	//pr_info("    *offset = 0x%08X\n", (uint32_t)*offset);
	...
	if((mem_index != 0) &&(mem_index != 1))// wait for idle mem
	{
		...
		while (cur_irq_cnt == get_current_irq_count()) {
			...
			{
				pr_info("f2smr_map_dev_read get interrupt but DeQueueF2sm error, exit\n");
				return -ERESTARTSYS;				
			}
	...
	up(&dev->sem);
	//pr_info("f2smr_map_dev_read exit\n");
	return count;
}


static int f2smr_ram_dev_open(struct inode *ip, struct file *fp)
{
	struct f2smr_ram_dev *dev = &the_f2smr_ram_dev;
	//pr_info("f2smr_ram_dev_open enter\n");
	//pr_info("ip = 0x%08X\n", (uint32_t)ip);
	//pr_info("fp = 0x%08X\n", (uint32_t)fp);
	...
	EnQueueF2sm(&g_avail_membuf_queue,0);
	//EnQueueF2sm(&g_avail_membuf_queue,1);     // 20201226 - current test time, only one memory used by FPGA, no PINGPONG yet
	the_f2smr_ram_dev.cur_io_buf_index = 0;

	up(&dev->sem);
	//pr_info("f2smr_ram_dev_open exit\n");
	return 0;
}

static int f2smr_ram_dev_release(struct inode *ip, struct file *fp)
{
	struct f2smr_ram_dev *dev = fp->private_data;
	//pr_info("f2smr_ram_dev_release enter\n");
	//pr_info("ip = 0x%08X\n", (uint32_t)ip);
	//pr_info("fp = 0x%08X\n", (uint32_t)fp);
	...
	up(&dev->sem);
	//pr_info("f2smr_ram_dev_release exit\n");
	return 0;
}

#if 0
long f2smr_ram_dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int retval = 0;
	/*
	 * work
	...
	case _IOC_F2SMR_DQBUF:
	{
		...
		if (!list_empty(&g_mem_queue)) 
		{
			...
			the_f2smr_ram_dev.cur_io_buf = ep->mem_buf;
		}
		else
		{
			/* code */
	...
	case _IOC_F2SMR_START:
	case _IOC_F2SMR_STOP:
	case _IOC_F2SMR_PAUSE:
		break;
...
static const struct file_operations f2smr_ram_dev_fops = {
	.owner = THIS_MODULE,
	.open = f2smr_ram_dev_open,
	.release = f2smr_ram_dev_release,
	.write = f2smr_ram_dev_write,
	.read = f2smr_ram_dev_read,
	//.unlocked_ioctl = f2smr_ram_dev_ioctl,
};
...
irqreturn_t f2smr_driver_interrupt_handler(int irq, void *data)
{
	...
	//the_f2smr_ram_dev.cur_io_buf = the_f2smr_ram_dev.p_uio_info->mem[0].internal_addr;
...
static int platform_probe(struct platform_device *pdev)
{
	...
	ret_val = request_irq(priv->mem0_irq,
			      f2smr_driver_interrupt_handler,
			      0,
			      "fpgar-mem0-irq",
			      priv);
	...
	the_f2smr_ram_dev.p_uio_info = &priv->the_uio_info;
	//the_f2smr_ram_dev.cur_io_buf = priv->the_uio_info.mem[0].internal_addr;
	the_f2smr_ram_dev.cur_io_buf_index = 0;
#endif
...
static int platform_remove(struct platform_device *pdev)
{
	...
	//release_mem_region(g_f2smr_driver_base_addr, g_f2smr_driver_size);
	if(priv->the_uio_info.mem[0].internal_addr)
	{
		priv->the_uio_info.mem[0].internal_addr = NULL;
	}
