...
{	
	...
	if (sigaction(SIGIO, &act, &oact)) {
		fprintf(stderr, "Setup Signal Error at line %d, file %s\n", __LINE__, __FILE__);
		return -1;
	}
	/* 通知驱动当前进程id和启动异步通知 */
	int oflag;
	fcntl(g_print_info.down_fd, F_SETOWN, getpid());
	oflag = fcntl(g_print_info.down_fd, F_GETFL);
	fcntl(g_print_info.down_fd, F_SETFL, oflag | FASYNC);
	/* 修改打印参数模拟异常或告警 */
	print_offset_regs[0] = DEFAULT_REG_VAL;
	print_offset_regs[1] = DEFAULT_REG_VAL;
	print_offset_regs[2] = DEFAULT_REG_VAL;
	print_offset_regs[3] = DEFAULT_REG_VAL;
	print_offset_regs[4] = DEFAULT_REG_VAL;
	print_offset_regs[5] = DEFAULT_REG_VAL;
	print_offset_regs[6] = DEFAULT_REG_VAL;
	print_offset_regs[7] = DEFAULT_REG_VAL;	
#endif	
...
#ifdef SIGNAL_TEST
	fcntl(g_print_info.down_fd, F_SETFL, oflag);
#endif	
