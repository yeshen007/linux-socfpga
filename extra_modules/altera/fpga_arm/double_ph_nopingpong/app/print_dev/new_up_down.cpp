...
void *down_thread(void *arg)
{
	...
	for (down_i = 1; down_i < m/down_blks_one_time; down_i++) {
		...
		if ((u32)ret != sizeof(read_info)) {	//为了和up保持代码一致，硬件不出问题不会在下行期间来结束打印中断
			if (ret == 1) {
				printf("down read normal stop, index %d\n", down_i);
				return (void *)1;
			} else if (ret ==2) {
				printf("down read accident stop, index %d\n", down_i);
				return NULL;
			} else {
				printf("down read wrong, index %d\n", down_i);
				return NULL;
			}
		}	
		if (read_info.mem_index != 0) {
	...
	ret = read(down_fd, &read_info, sizeof(read_info));
	if (ret == 1) {
		fprintf(stderr, "down read normal stop\n");
		return (void *)1;
	} else if (ret ==2) {
		fprintf(stderr, "down read accident stop\n");
		return NULL;
	} else {
		fprintf(stderr, "down read wrong\n");
		return NULL;
	}	
