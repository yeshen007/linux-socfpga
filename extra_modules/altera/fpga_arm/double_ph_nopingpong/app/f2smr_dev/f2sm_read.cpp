...
#define SEED_OFFSET 69
/*
	unsigned int randv = (unsigned int)rand();
	unsigned int ost_ph1_randv = randv;
	unsigned int ost_ph2_randv = ost_ph1_randv + SEED_OFFSET;
	unsigned int tnd_ph1_randv = randv;
	unsigned int tnd_ph2_randv = tnd_ph1_randv + SEED_OFFSET;
...
//#define YXMTEST
#ifdef YXMTEST
	memset(ost_pPrbsBuff, 0, ost_size);
	memset(tnd_pPrbsBuff, 0, tnd_size);
#endif	
	...
	while (1) {
		...
		if (nRead == sizeof(read_info)) {		//read ok
			if (first) {
				printf("first read info,must won't wait\n");
				first = false;
				//times++;
			} else {
				...
				/* gen seed and rand data */
/*				
				randv = (unsigned int)rand();
				ost_ph1_randv = randv;
				ost_ph2_randv = ost_ph1_randv + SEED_OFFSET;
				tnd_ph1_randv = randv;
				tnd_ph2_randv = tnd_ph1_randv + SEED_OFFSET;
				printf("rand v: 0x%08x\n", randv);
*/
