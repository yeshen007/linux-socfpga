...
void do_pingpong_test(CF2smIntf &f2sm, int frb_count, char *pDataBuf,int nLimit,int randv)
{
	...
	while (cnt < 5) {
		...
		/* 启动dma传输 */
        f2sm.StartTransfer(read_info.mem_index, min_l, min_h, new_frb_count);	
...
int pingpong_test(void)
{
    ...
    //do_pingpong_test(f2sm, 1024, pDataBuf, nLimit, randv);
    do_pingpong_test(f2sm, 512, pDataBuf, nLimit, randv);
	/* 释放m_fd和virtual_base */
