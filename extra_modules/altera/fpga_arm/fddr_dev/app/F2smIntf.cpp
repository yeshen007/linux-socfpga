...
#define FDDR_DEV "/dev/fddr_io_dev"
...
void CF2smIntf::ResetErrCounter()
{
    ...
    reg_addr = __IO_CALC_ADDRESS_NATIVE(virtual_base,F2SM_FDDR_ERESET_REG);
...
void CF2smIntf::StartWrite(int seed)
{
    ...
    reg_addr = __IO_CALC_ADDRESS_NATIVE(virtual_base,F2SM_FDDR_SEED_REG);
    alt_write_word(reg_addr,  (unsigned int)seed);

    reg_addr = __IO_CALC_ADDRESS_NATIVE(virtual_base,F2SM_FDDRW_START_REG);
...
{
    void * reg_addr =  NULL;
    reg_addr = __IO_CALC_ADDRESS_NATIVE(virtual_base,F2SM_FDDRR_START_REG);
...
bool CF2smIntf::CheckResult()  // Read result
{
    ...
    void * reg_addr = __IO_CALC_ADDRESS_NATIVE(virtual_base,F2SM_FDDR_ERR_IND_REG);
    result = alt_read_word(reg_addr);
    errind = result & _F2SM_FDDR_ERR_IND_MASK;

    reg_addr = __IO_CALC_ADDRESS_NATIVE(virtual_base,F2SM_FDDR_ERR_COUNT_REG);
    result = alt_read_word(reg_addr);
...
{
    unsigned int result = 0;
    void * reg_addr = __IO_CALC_ADDRESS_NATIVE(virtual_base,F2SM_FDDR_SYSCLOCK_RDY_REG);
    result = alt_read_word(reg_addr);
    ...
    reg_addr = __IO_CALC_ADDRESS_NATIVE(virtual_base,F2SM_FDDR_DDR_RDY_REG);
    result = alt_read_word(reg_addr);
