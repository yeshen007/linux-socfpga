...
static int dp83869_config_intr(struct phy_device *phydev)
{
	...
	if (phydev->interrupts == PHY_INTERRUPT_ENABLED) {
		...
		micr_status |= (MII_DP83869_MICR_AN_ERR_INT_EN |
...
#endif
static int dp83869_set_wol(struct phy_device *phydev, struct ethtool_wolinfo *wol)
{
	...
	if (wol->wolopts & (WAKE_MAGIC | WAKE_MAGICSECURE | WAKE_UCAST | WAKE_BCAST)) {
		val_rxcfg |= DP83869_WOL_ENH_MAC;
		val_micr |= MII_DP83869_MICR_WOL_INT_EN;

		if (wol->wolopts & WAKE_MAGIC || wol->wolopts & WAKE_MAGICSECURE) {
			mac = (u8 *)ndev->dev_addr;

			if (!is_valid_ether_addr(mac))
				return -EINVAL;

			ret = phy_write_mmd(phydev, DP83869_DEVADDR, DP83869_RXFPMD1, mac[1] << 8 | mac[0]);
			if (ret)
				return ret;

			ret = phy_write_mmd(phydev, DP83869_DEVADDR, DP83869_RXFPMD2, mac[3] << 8 | mac[2]);
			if (ret)
				return ret;

			ret = phy_write_mmd(phydev, DP83869_DEVADDR, DP83869_RXFPMD3, mac[5] << 8 | mac[4]);
			if (ret)
				return ret;
		...
		if (wol->wolopts & WAKE_MAGICSECURE) {
			ret = phy_write_mmd(phydev, DP83869_DEVADDR, DP83869_RXFSOP1, (wol->sopass[1] << 8) | wol->sopass[0]);
			if (ret)
				return ret;

			ret = phy_write_mmd(phydev, DP83869_DEVADDR, DP83869_RXFSOP2, (wol->sopass[3] << 8) | wol->sopass[2]);
			if (ret)
				return ret;
			ret = phy_write_mmd(phydev, DP83869_DEVADDR, DP83869_RXFSOP3, (wol->sopass[5] << 8) | wol->sopass[4]);
			if (ret)
				return ret;
               ...
               struct ethtool_wolinfo *wol)
{
   ...
   wol->supported = (WAKE_UCAST | WAKE_BCAST | WAKE_MAGIC | WAKE_MAGICSECURE);
   wol->wolopts = 0;
   ...
   if (value & DP83869_WOL_SEC_EN) {
       ...
       sopass_val = phy_read_mmd(phydev, DP83869_DEVADDR, DP83869_RXFSOP2);
       if (sopass_val < 0) {
       ...
       sopass_val = phy_read_mmd(phydev, DP83869_DEVADDR, DP83869_RXFSOP3);
       if (sopass_val < 0) {
...
static int dp83869_set_downshift(struct phy_device *phydev, u8 cnt)
{
   ...
   if (!cnt)
       return phy_clear_bits(phydev, DP83869_CFG2, DP83869_DOWNSHIFT_EN);
   ...
   case DP83869_DOWNSHIFT_8_COUNT:
       ...
       break;
   default:
       phydev_err(phydev, "Downshift count must be 1, 2, 4 or 8\n");
       return -EINVAL;
   }
   ...
   return phy_modify(phydev, DP83869_CFG2, DP83869_DOWNSHIFT_EN | DP83869_DOWNSHIFT_ATTEMPT_MASK, val);
}
...
static int dp83869_config_port_mirroring(struct phy_device *phydev)
{
	...
	if (dp83869->port_mirroring == DP83869_PORT_MIRRORING_EN)
		return phy_set_bits_mmd(phydev, DP83869_DEVADDR, DP83869_GEN_CFG3, DP83869_CFG3_PORT_MIRROR_EN);
	else
 		return phy_clear_bits_mmd(phydev, DP83869_DEVADDR, DP83869_GEN_CFG3, DP83869_CFG3_PORT_MIRROR_EN);
}
...
static int dp83869_of_init(struct phy_device *phydev)
{
	...
	ret = of_property_read_u32(of_node, "ti,clk-output-sel", &dp83869->clk_output_sel);
	...
	if (ret == 0) {
		if (dp83869->mode < DP83869_RGMII_COPPER_ETHERNET || dp83869->mode > DP83869_SGMII_COPPER_ETHERNET)
			return -EINVAL;
	} else {
    ...
    dev_info(dev, "dp83869_of_init dp83869->mode is %d\n", dp83869->mode);
	...
	if (of_property_read_bool(of_node, "enet-phy-lane-swap")) {
		...
		if (ret < 0)
        {   
            dev_err(dev, "dp83869 phy_read_mmd DP83869_STRAP_STS1 error\n");
			return ret;
		}
	...
	if (of_property_read_u32(of_node, "rx-fifo-depth", &dp83869->rx_fifo_depth))
		dp83869->rx_fifo_depth = DP83869_PHYCR_FIFO_DEPTH_4_B_NIB;

	if (of_property_read_u32(of_node, "tx-fifo-depth", &dp83869->tx_fifo_depth))
		dp83869->tx_fifo_depth = DP83869_PHYCR_FIFO_DEPTH_4_B_NIB;
	
	dev_info(dev, "~~~~~~~~~~dp83869 dp83869->rx_fifo_depth is %d\n", dp83869->rx_fifo_depth);
    	dev_info(dev, "~~~~~~~~~~dp83869 dp83869->tx_fifo_depth is %d\n", dp83869->tx_fifo_depth);
#ifdef VERSION_ERROR
	dp83869->rx_int_delay = phy_get_internal_delay(phydev, dev, &dp83869_internal_delay[0], delay_size, true);
	if (dp83869->rx_int_delay < 0)
		dp83869->rx_int_delay = dp83869_internal_delay[DP83869_CLK_DELAY_DEF];

	dp83869->tx_int_delay = phy_get_internal_delay(phydev, dev, &dp83869_internal_delay[0], delay_size, false);
	if (dp83869->tx_int_delay < 0)
		dp83869->tx_int_delay = dp83869_internal_delay[DP83869_CLK_DELAY_DEF];
#endif
	return ret;
}
#else
				   ...
				   struct dp83869_private *dp83869)
{
	...
	if (dp83869->io_impedance >= 0)
		ret = phy_modify_mmd(phydev, DP83869_DEVADDR, DP83869_IO_MUX_CFG, DP83869_IO_MUX_CFG_IO_IMPEDANCE_CTRL,
				                dp83869->io_impedance & DP83869_IO_MUX_CFG_IO_IMPEDANCE_CTRL);

	return ret;
}

static int dp83869_configure_fiber(struct phy_device *phydev, struct dp83869_private *dp83869)
{
...
static int dp83869_configure_mode(struct phy_device *phydev, struct dp83869_private *dp83869)
{
	...
	phy_ctrl_val = (dp83869->rx_fifo_depth << DP83869_RX_FIFO_SHIFT | dp83869->tx_fifo_depth << DP83869_TX_FIFO_SHIFT 
	...
	case DP83869_RGMII_SGMII_BRIDGE:
		ret = phy_modify_mmd(phydev, DP83869_DEVADDR, DP83869_OP_MODE, DP83869_SGMII_RGMII_BRIDGE, DP83869_SGMII_RGMII_BRIDGE);
		if (ret)
			return ret;
...
static int dp83869_config_init(struct phy_device *phydev)
{
...
#ifdef TEST_CL
	ret = phy_modify(phydev, DP83869_CFG2, DP83869_DOWNSHIFT_EN, DP83869_DOWNSHIFT_EN);
	if (ret)
		return ret;
#endif
	...
	if (ret)
    {   
        dev_err(dev, "dp83869_config_init dp83869_configure_mode error\n");
		return ret;
	}
	...
	if (dp83869->clk_output_sel != DP83869_CLK_O_SEL_REF_CLK)
		ret = phy_modify_mmd(phydev,
				     ...
				     dp83869->clk_output_sel << DP83869_IO_MUX_CFG_CLK_O_SEL_SHIFT);

	if (phy_interface_is_rgmii(phydev)) {
#ifdef VERSION_ERROR
		ret = phy_write_mmd(phydev, DP83869_DEVADDR, DP83869_RGMIIDCTL,
                                dp83869->rx_int_delay |dp83869->tx_int_delay << DP83869_RGMII_CLK_DELAY_SHIFT);
		if (ret)
			return ret;
#endif
		...
		ret = phy_write_mmd(phydev, DP83869_DEVADDR, DP83869_RGMIICTL, val);
	}
#endif
	return ret;
}
...
static int dp83869_probe(struct phy_device *phydev)
{
	...
	dp83869 = devm_kzalloc(&phydev->mdio.dev, sizeof(*dp83869),GFP_KERNEL);
	if (!dp83869)
		return -ENOMEM;
